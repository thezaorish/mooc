...
def mapper():
    ...
    for line in reader:
        fullBody = line[4]
        periodOcc = fullBody.count(".", 0, len(fullBody))
        exclOcc = fullBody.count("!", 0, len(fullBody))
        questionOcc = fullBody.count("?", 0, len(fullBody))

        if periodOcc + exclOcc + questionOcc > 1:
            continue
        if periodOcc == 1 and fullBody.index(".") < len(fullBody):
            continue
