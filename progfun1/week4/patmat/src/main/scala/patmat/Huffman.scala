...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    val zero: Map [Char, Int] = Map().withDefaultValue (0)
    chars.foldLeft(zero)((occurrences, letter) => {
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
    freqs.sortWith((tuple1, tuple2) => { // sort by frequencies
      tuple1._2 < tuple2._2
    })
      .map(tuple => { // map to Leaf
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = {
    if (trees.length < 2) trees
      ...
      val tree: Fork = makeCodeTree(trees(0), trees(1))
      trees.tail.tail.::(tree).sortWith(weight(_) < weight(_))
    }
  }
   ...
   */
  def until(c: List[CodeTree] => Boolean, f: List[CodeTree] => List[CodeTree])(trees: List[CodeTree]): CodeTree = {
    if (c(trees)) trees(0)
    else until(c, f)(f(trees))
  }
